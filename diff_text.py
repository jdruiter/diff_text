...
from __future__ import unicode_literals  # "" will become u"" automatically
...
""" Diff text: diff two texts 

Computes the difference between two texts and returns output as pretty html

Forked from google's diff_match_patch.py
Added helperfunctions to quickly get output printable in page/email
Match and patch functions are removed for brevity.
Author: Joris de Ruiter <joris@jdruiter.nl>
...
def similar(a, b):
    """ Calculates the similarity between two texts or strings.
    Use to prevent calculating the exact different between a text that differs only
    by a small change (e.g. a single space or dot)
    Example use:
      if similar(text1, text2) < 0.99:
        diff_match_patch().htmldiff(text1, text2)

    >>> similar('aap', 'noot'): 0.0
    >>> similar('aap', 'aap'): 1.0
    """
...
def htmldiff(text1, text2, strip_html=False):
    """
    Ready to use method that shows the diff between two texts as pretty html.
    Input text can be plain or html text.
    To remove html tags before comparison, use @strip_html

    Args:
    text1: Old string to be diffed.
    text2: New string to be diffed.
    strip_html: strip html tags before parsing

    Example call: htmldiff(text1, text2, True)
    ...
    if text1 and not text2:
        return "<del style='background:#ffe6e6;'>{}</del>".format(text1)
    elif text2 and not text1:
        return "<ins style='background:#e6ffe6;'>{}</ins>".format(text2)
    ...
    for (op, data) in diffs:
        ...
        if op == dmp.DIFF_INSERT:
            ...
            html.append("<del style='background: #ffe6e6;'>{}</del>".format(text))  #+style="text-decoration:line-through;"
...
class diff_match_patch:
    ...
    def diff_main(self, text1, text2, checklines=True, deadline=None):
        ...
        if not deadline:
            ...
            if self.Diff_Timeout <= 0:  # infinite
                deadline = sys.maxint
            else:
